...
/// Physical address of the console UART receive register. Reading it returns the next byte of
/// host input, blocking until one is available; 0 on end of input.
pub const UART_RX: u32 = 0x10000004;
...
pub const Machine = struct {
    ...
    pub fn read8(self: *Machine, a: u32) ?u8 {
        if (a == UART_RX) return std.io.getStdIn().reader().readByte() catch 0;
